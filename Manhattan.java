...
public class Manhattan {
	...
	public static int ManhattanDistance(int x1,int y1,int x2,int y2) {	//Calculates the Manhattan Distance between two nodes
    	int distance = Math.abs(x2 - x1) + Math.abs(y2 - y1);	//distance = |x2-x1|+|y2-y1|
    	return distance;
	}
	...
	public static void main(String[] args) {
		...
		int od,ud,bd,fd;	//distance from the neighbors to the end
		int mind;	//min distance from the neighbors to the end
		...
		try {
		    ...
		    while (cx != xe || cy!= ye) {
				...
				if (ox!=-1) {	
					od = ManhattanDistance(xe,ye,ox,oy);	//calculate the distance from the over node to the end
				}else {
					od = 200;	//value that cannot be take because max table is 99*99
				}
				if (ux!=-1) {	
					ud = ManhattanDistance(xe,ye,ux,uy);	//calculate the distance from the under node to the end
				}else {
					ud = 200;	//value that cannot be take because max table is 99*99
				}
				if (bx!=-1) {
					bd = ManhattanDistance(xe,ye,bx,by);	//calculate the distance from the before node to the end
				}else {
					bd = 200;	//value that cannot be take because max table is 99*99
				}
				if (fx!=-1) {
					fd = ManhattanDistance(xe,ye,fx,fy);	//calculate the distance from the front node to the end
				}else {
